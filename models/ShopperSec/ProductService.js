...
myrouter.get('/Dashboard/productUpload', checkAuthenticated, async function (req, res) {
    ...
    db.getDB().collection("EMPLOYEE_ID").findOne({
        email: email
    })
        .then(data => {
            if (data) {
...
myrouter.post('/Dashboard/productUpload', checkAuthenticated, async (req, res) => {
    ...
    fileUploadCtrl(req, res, async function (err) {
        if (err) {
            ...
            employee_details = await db.getDB().collection("EMPLOYEE_ID").findOne({
                email: email
            })
                .then(async (emp_det) => {
                    if (emp_det) {
                ...
                .then(async (T_counts) => {
                    ...
                    db.getDB().collection("EMPLOYEE_ID").updateOne(countquery, countsUpdate)
